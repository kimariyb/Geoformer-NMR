...
class CarbonDataset(InMemoryDataset):
    ...
    def process(self):
        ...
        for mol in tqdm(suppl, desc="Processing carbon dataset", unit="mol", ncols=100, total=len(suppl)):            
            ...
            data.pos = torch.tensor(graph['pos'], dtype=torch.float)
            data.z = torch.tensor(graph['z'], dtype=torch.long)
    ...
    def mol2graph(self, mol):
        r"""
        Convert RDKit molecule object to graph.
        ...
        try:
            ...
            for atom in mol.GetAtoms():
                node_feat.append(atom_to_feature_vector(atom))
            ...
            for bond in mol.GetBonds():
                ...
                edge_feat.append(bond_to_feature_vector(bond))
                edge_feat.append(bond_to_feature_vector(bond))
