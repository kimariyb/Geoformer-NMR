...
class GeoformerMultiHeadAttention(nn.Module):
    ...
    def reset_parameters(self):
        nn.init.kaiming_normal_(self.k_proj.weight, a=math.sqrt(5), mode="fan_in", nonlinearity="relu")
        self.k_proj.bias.data.fill_(0.0)
        nn.init.kaiming_normal_(self.q_proj.weight, a=math.sqrt(5), mode="fan_in", nonlinearity="relu")
        self.q_proj.bias.data.fill_(0.0)
        nn.init.kaiming_normal_(self.v_proj.weight, a=math.sqrt(5), mode="fan_in", nonlinearity="relu")
        self.v_proj.bias.data.fill_(0.0)
        nn.init.kaiming_normal_(self.dk_proj.weight, a=math.sqrt(5), mode="fan_in", nonlinearity="relu")
        self.dk_proj.bias.data.fill_(0.0)
        nn.init.kaiming_normal_(self.du_update_proj.weight, a=math.sqrt(5), mode="fan_in", nonlinearity="relu")
        self.du_update_proj.bias.data.fill_(0.0)
        nn.init.kaiming_normal_(self.dihedral_proj.weight, a=math.sqrt(5), mode="fan_in", nonlinearity="relu")
        nn.init.kaiming_normal_(self.edge_attr_update.weight, a=math.sqrt(5), mode="fan_in", nonlinearity="relu")
...
class GeoformerAttnBlock(nn.Module):
    ...
    def reset_parameters(self):
        ...
        nn.init.kaiming_normal_(self.ffn[0].weight, a=math.sqrt(5), mode="fan_in", nonlinearity="relu")
        self.ffn[0].bias.data.fill_(0.0)
        nn.init.kaiming_uniform_(self.ffn[3].weight, a=math.sqrt(5), mode="fan_in", nonlinearity="relu")
...
class GeoformerEncoder(nn.Module):
    ...
    def reset_parameters(self):
        ...
        nn.init.kaiming_normal_(self.dist_proj.weight, a=math.sqrt(5), mode="fan_in", nonlinearity="relu")
...
class GeoformerSpectraRegression(PreTrainedModel):
    def __init__(self, config, *inputs, **kwargs):
        ...
        self.embedding_dim = config.embedding_dim
        self.pred_hidden_dim = config.pred_hidden_dim
    ...
    def forward(
        ...
        **kwargs
        ...
        node_embed_feats, _ = self.geo_encoder(z=z, pos=pos) # (B, N, F)
        pred = self.readout_n(node_embed_feats)
        pred = node_embed_feats[mask]  
