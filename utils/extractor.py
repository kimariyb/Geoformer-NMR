...
def mask_hydrogen(x: torch.Tensor):
    r"""
    mask the hydrogen atoms in the input tensor
...
def mask_others(x: torch.Tensor):
    r"""
    mask the other atoms in the input tensor
...
def mask_inferences(x: torch.Tensor):
    r"""
    mask the inference atoms in the input tensor
...
def genrate_mask(mol: Chem.Mol, shift_dict: Dict):
    r"""
    generate the mask for the input molecule
    ...
    mol : Chem.Mol
        the input molecule
    shift_dict : Dict
        the dictionary of chemical shift values
    ...
    label_tensor = torch.zeros(num_atoms, 2)
    ...
    # mask the hydrogen atoms, other atoms and inference atoms
    hydrogen_mask_tensor = mask_hydrogen(label_tensor)
    label_tensor = label_tensor[hydrogen_mask_tensor]
    
    others_mask_tensor = mask_others(label_tensor)
    inferences_mask_tensor = mask_inferences(label_tensor)
    
    return label_tensor, others_mask_tensor, inferences_mask_tensor
    

def extract_carbonShift(mol: Chem.Mol):
    r"""
    提取 13C 化学位移信息
    """
    # 获取分子的属性字典
...
def extract_hydrogenShift(mol: Chem.Mol):
    r"""
    提取 1H 化学位移信息
    """
    # 获取分子的属性字典
...
def extract_fluorineShift(mol: Chem.Mol):
    r"""
    提取 19F 化学位移信息
    """
    # 获取分子的属性字典
